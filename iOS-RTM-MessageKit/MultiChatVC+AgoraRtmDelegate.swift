...
extension MultiChatVC: AgoraRtmChannelDelegate, AgoraRtmDelegate {
    ...
    func handleMessageReceived(message: AgoraRtmMessage) {
        ...
        if message.type == .text, let json = try? JSONSerialization.jsonObject(
            with: data, options: []
        ) as? [String: Any] {
            ...
            switch type {
            case "status":
                guard let userStatus = RTMUser.Status(rawValue: body) else {
                    print("could not set status to \(body)")
                    return
                }
    ...
    func channel(_ channel: AgoraRtmChannel, memberJoined member: AgoraRtmMember) {
        channel.kit.send(self.localUser.statusRTMMessage, toPeer: member.userId, completion: { sentErr in
            if sentErr != .ok {
